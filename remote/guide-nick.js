		...
		$.post("/api/publish", post_data).done(function (response) {
			...
			if (frames == 0) {
				...
				if (subscriberIds.length == 0) {
					var html = '<li style="width: 200px" class="noAvailable" onclick = "void(0)"><div style="position: relative; top: 50%; transform: translateY(-50%);	-webkit-transform: translateY(-50%);"><p>No available players</p></div></li>'
					$("#frames").append(html)
				}
			} else {
				$("#frames").html("")
		...
		$.get("networks-olympics.json")
			.done(function (response) {
				...
				networks.forEach(function (network, index, networkarray) {
					...
					if (image) {
						html += '<a href="' + (urls ? urls[0].url : "") + '" class="network ' + name + '" src="' + (urls ? urls[0].url : "") + '" alt="' + alt + '" style="float: left;"><div class="image" target="_hsb"><div><img src="' + image + '" alt="' + alt + '"/></div></div></a>'


					} else {
						html += '<a href="' + (urls ? urls[0].url : "") + '" class="network ' + name + '" src="' + (urls ? urls[0].url : "") + '" alt="' + alt + '" style="width: 100%; height: 70px; text-align: center" target="_hsb"><p style="color: ' + color + '; position: relative; text-align: center; top: 50%; transform: translateY(-50%); -webkit-transform: translateY(-50%); width: 100%;">' + alt + '</p></a>'
					...
					if (urls) {
						urls.forEach(function (element, index, urlsarray) {
							...
							html += '<a href="' + url + '" class="network ' + name + '" src="' + url + '" alt="' + alt + ' target="_hsb"><div><p style="color: ' + color + '">' + source + '</p>'
		...
		nbcOlympics.done(function (response) {
			...
			if ($.isArray(response.programs)) {
				response.programs.forEach( function(currentValue, index, array) {
					...
					html += '<a href="' + url + '" class="network ' + name + '" src="' + url + '" alt="' + alt + '" style="float: left;" target="_hsb"><div class="image"><div><img src="' + image + '" alt="' + alt + '"/></div></div></a>'
